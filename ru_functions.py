...
try:
	with open('save.json', 'r') as fh:
		...
		print('Обнаружено сохранение. Ник(имя): {}, компания: {}, состояние: ${}, мастерская:{}'.format(nickname, company, money, workshop))
except:
	nickname = input('Введите ник: ')
	company = input('Введите компанию: ')
	achievements.append('My opportunity')
	print('Вы получили достижение: Мои возможности!')
	dict = {}
	dict['achievements'] = achievements
	with open('achievements.json', mode='w') as f:
		json.dump(dict, f)
...
def money_help():
	...
	with open('achievements.json', mode='w') as f:
		json.dump(dict, f)
	print('Вы получили достижение: Заткнись и возьми мои деньги!')
	return None
...
def virus():
	...
	if a_v == c_1:
		...
		if a_v_c == yes:
			...
			print('Вы получили достижение: Мой первый взлом')
			...
			if res == int('0'):
				...
				if reaction == int('2'):
					...
					print('Вы получили достижение: я очень удачливый')
					...
					if money > count:
						...
						print('Вы получили достижение: Взлом = конец')
						time.sleep(10)
						quit()
					elif money == count:
				...
				print('Вы получили достижение: Я про-хакер')
				time.sleep(2)
				try:
...
def works():
	...
	class work:
		...
		print('Вы получили достижение: Работа - лучшая практика')
		if a == w1:
...
def save_achievement():
	...
	with open('achievements.json', mode='w') as f:
		json.dump(dict, f)
	time.sleep(5)
	print('Достижения сохранены')

def new_car():
	achievements.append('My first car')
	print('Вы получили достижение: Моя первая машина')
	global workshop
	global count_cars
	count_cars = int('0')
	try:
	...
	def compilar_car():
		...
		if car_answer == yes:
			...
			if money < int('0'):
				...
				print('Вы получили достижение: Банкот - моё второе имя')
